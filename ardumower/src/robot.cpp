...
// watchdog reset detect signature
...
String rfid_list[MAX_RFIDS][8] = {
    {"44907165", "BACKTOSTATION", "RTS", "240", "75", "0", "0", "0"},
    {"813FFA64", "BACKTOSTATION", "RTS", "240", "85", "0", "0", "0"},
    {"A7D725BE", "BACKTOSTATION", "RTS", "240", "49", "0", "0", "0"},
    {"455326BE", "BACKTOSTATION", "RTS", "240", "35", "0", "0", "0"},
    {"719926BE", "BACKTOSTATION", "RTS", "240", "95", "0", "0", "0"},
    {"8EA126BE", "BACKTOSTATION", "RTS", "220", "-10", "0", "0", "0"},
    {"96D52480", "BACKTOSTATION", "SPEED", "135", "0", "60", "0", "0"}};
...
*/
// disable/enable watchdog mower.h line 129 -> #define Enable_DueWatchdog false/true
#if Enable_DueWatchdog
...
void Robot::loadSaveRobotStats(bool readflag)
{
...
void Robot::loadSaveErrorCounters(bool readflag)
{
  if (readflag)
    Console.println(F("Load ErrorCounters"));
  else
    Console.println(F("Save ErrorCounters"));
...
void Robot::loadSaveUserSettings(bool readflag)
{
  ...
  eereadwrite(readflag, addr, UseBumperDock); // Use bumper on docking
...
void Robot::autoReboot()
{
  ...
  if (RaspberryPIUse)
  {
    Console.println(F("Due reset after 1 seconds, send a command to Pi for restart also"));
...
void Robot::setRemotePPMState(unsigned long timeMicros, bool remoteSpeedState, bool remoteSteerState, bool remoteMowState, bool remoteSwitchState)
{
...
void Robot::setMotorMowPWM(int pwm, bool useAccel)
{
...
void Robot::setMotorPWM(int pwmLeft, int pwmRight, bool useAccel)
{
  ...
  */
  ...
  if (useAccel)
  {
    ...
    //   Console.println("motorLeftPWMCurr 255!");
    // }
...
void Robot::checkButton()
{
  ...
  bool buttonPressed = (readSensor(SEN_BUTTON) == LOW);
...
void Robot::newTagFind()
{
  if (millis() >= nextTimeSendTagToPi)
  {
    ...
    Console.print("New RFID tag found. ID: ");
    Console.println(rfidTagFind);
    if ((rfidUsePi) && (RaspberryPIUse))
      MyRpi.SendRfidToPi();
    if (rfidUseDue)
    {
      // [14:18:45 165.543013] New RFID tag found. ID: 96D52480
      // [14:18:45 0.004195] RFID tag ToDO is: SPEED
      // [14:18:45 0.005097] Change tracking speed tag. New speed is 135next 0 cm.
      ...
      if (newtagToDo == "Null")
      {
        Console.print("Tag id: ");
        Console.print(temp_tag);
        Console.print(" or current status: ");
        Console.print(temp_status);
        Console.println(" not match to database.");
      }
      else
      {
        ...
        if (newtagToDo == "RTS") //return to station from station area
        {
          ...
          Console.print(" degrees and new speed is ");
...
void Robot::readSensors()
{
  ...
  if (millis() >= nextTimeMotorSense)
  {
    ...
    if (secondMowMotor)
      motor2MowSenseADC = readSensor(SEN_MOTOR2_MOW); // mow motor 2
    ...
    if (secondMowMotor)
      motor2MowSenseCurrent = motor2MowSenseCurrent * (1.0 - accel) + ((double)motor2MowSenseADC) * motor2MowSenseScale * accel; // mow motor 2
    ...
    if (batVoltage > 8)
    {
      ...
      if (secondMowMotor)
        motor2MowSense = motor2MowSenseCurrent * batVoltage / 1000; // mow motor 2
    }
    else
    {
      ...
      if (secondMowMotor)
        motor2MowSense = motor2MowSenseCurrent * batFull / 1000; // mow motor 2
    }
...
void Robot::setNextState(byte stateNew, byte dir)
{
  ...
  case STATE_FORWARD_ODO:
    ...
    { // it s a new line so don't limit the forward distance and invert the rollDir
      if ((stateLast != STATE_STOP_CALIBRATE) && (mowPatternCurr == MOW_LANES))
      { // change the rolldir only in lane mowing and not after calibration of imu
        if (rollDir == 0)
          rollDir = 1;
        else
          rollDir = 0;
  ...
  case STATE_STATION_FORW: //when start in auto after mower  roll this state accel the 2 wheel before forward
    ...
    stateEndOdometryRight = odometryRight + (int)(odometryTicksPerCm * stationForwDist); //60CM to accel
    stateEndOdometryLeft = odometryLeft + (int)(odometryTicksPerCm * stationForwDist);
    OdoRampCompute();
    //motorMowEnable = true;
    break;
...
void Robot::checkCurrent()
{
  ...
  { //do not start the spirale if in tracking and motor detect high grass
    if ((motorMowEnable) && ((motor1MowSense >= 0.8 * motorMowPowerMax) || (motor2MowSense >= 0.8 * motorMowPowerMax)))
    {
      ...
      if (motor1MowSense >= 0.8 * motorMowPowerMax)
      {
        Console.print("Warning motor1MowSense >= 0.8 * motorMowPowerMax. Motor1: ");
        Console.print(motor1MowSense);
        Console.print(" W, MAX: ");
        Console.print(motorMowPowerMax);
        Console.println(" W");
      }
      if (motor2MowSense >= 0.8 * motorMowPowerMax)
      {
        Console.print("Warning motor2MowSense >= 0.8 * motorMowPowerMax. Motor2: ");
  ...
  if ((motorMowEnable) && (motor1MowSense >= motorMowPowerMax))
  {
  ...
  else
  {
    ...
    motor1MowSenseCounter = 0;
    if (secondMowMotor)
      motor2MowSenseCounter = 0; // second mow motor
...
void Robot::checkSonarPeriTrack()
{
  ...
  if (sonarDistRight < 30 || sonarDistRight > 150)
...
void Robot::readDHT22()
{
  ...
  if ((DHT22Use) && (millis() > nextTimeReadDHT22))
  {
    ...
    if (temperatureDht >= maxTemperature)
    {
      ...
      if ((raspiTempUse) && (RaspberryPIUse))
      {
...
void Robot::loop()
{
  ...
  //checkOdometryFaults();
  if (stateCurr == STATE_OFF)
    checkButton(); //read only when needed
  motorMowControl();
  checkTilt();
  ...
  case STATE_OFF:
    ...
    readDHT22();
    checkBattery();
    if ((Enable_DueWatchdog) && (timerUse) && (resetByWDT)) // check timer after wdt reset
      checkTimer();
    break;
  ...
  case STATE_STATION_CHARGING:
    ...
    if (batMonitor)
    {
      if ((chgCurrent < batFullCurrent) && (millis() - stateStartTime > 1800000)) //was 2000ms, now 30min
      {
        if (autoResetActive)
        {
      ...
      if ((millis() - stateStartTime) > (chargingTimeout * 60 * 60 * 1000)) // hours to ms
      {
    ...
    if (RaspberryPIUse)
    {
      ...
      {
        nextTimeChgRasPISendBat = millis() + 5000; // delay ms
